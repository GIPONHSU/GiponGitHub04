...
import groundImageSrc from '../../PIC/現代都市_含角色模擬_02.png';
import originalGroundImageSrc from '../../PIC/ChatGPT Image 2026年6月21日 下午01_13_45.png';
import stage2GroundImageSrc from '../../PIC/Gemini_Generated_Image_hkbptohkbptohkbp.png';

const groundImage = new Image();
groundImage.src = groundImageSrc;
const originalGroundImage = new Image();
originalGroundImage.src = originalGroundImageSrc;
...
export function drawArena(ctx: CanvasRenderingContext2D, engine: GameEngine) {
        ...
        const isOriginalBgReady = originalGroundImage.complete && originalGroundImage.naturalWidth > 0;
        const bgY = (engine.activeArenaCenterY ?? 540) - 540;
        
        if (isBgReady) {
            if (!engine.transitionBgChanged) {
                // Focus on the upper half region of the ground (first stage) image
                // Use original aspect ratio for 現代都市_含角色模擬_02.png (cover mode)
                const imgRatio = currentActiveBg.naturalWidth / currentActiveBg.naturalHeight;
                const targetRatio = CANVAS_W / CANVAS_H;
                let sW = currentActiveBg.naturalWidth;
                let sH = currentActiveBg.naturalHeight;
                let sX = 0;
                let sY = 0;

                if (imgRatio > targetRatio) {
                    sW = sH * targetRatio;
                    sX = (currentActiveBg.naturalWidth - sW) / 2;
                } else {
                    sH = sW / targetRatio;
                    sY = (currentActiveBg.naturalHeight - sH) / 2;
                }
                ctx.drawImage(currentActiveBg, sX, sY, sW, sH, 0, 0, CANVAS_W, CANVAS_H);
                
                // For the second area of stage 1 (y: 1080 to 2160), use the original ChatGPT image
                if (isOriginalBgReady) {
                    const origW = originalGroundImage.naturalWidth;
                    const origH = originalGroundImage.naturalHeight / 2;
                    ctx.drawImage(originalGroundImage, 0, origH, origW, origH, 0, CANVAS_H, CANVAS_W, CANVAS_H);
                }
            } else {
                ctx.drawImage(currentActiveBg, 0, bgY, CANVAS_W, CANVAS_H);
            }
        } else {
            ...
            if (isOriginalBgReady) {
                const origW = originalGroundImage.naturalWidth;
                const origH = originalGroundImage.naturalHeight / 2;
                ctx.drawImage(originalGroundImage, 0, origH, origW, origH, 0, bgY, CANVAS_W, CANVAS_H);
            }
        }
        ...
        if (!engine.transitionBgChanged) {
            ...
            ctx.fillRect(0, bgY, CANVAS_W, CANVAS_H);
        }
        ...
        decorativeStones.forEach(stone => {
            // Hide stones in Area 1 (Y < 1080)
            if (stone.y < 1080) return;
        ...
        engine.activeLaunchPads.forEach(pad => {
            const isMulti = !!pad.isMultiplayer;
            const padRadius = isMulti ? 120 : 48; // Multiplayer is 2.5x radius
            ...
            if (isMulti) {
                // Get the total number of alive human players (P-slots currently participating)
                const X = Math.max(1, engine.tops.filter(t => !t.isAI && !t.isExploding && t.hp > 0).length);
                // Get the number of players currently inside or waiting in the multiplayer pad
                const waitingTopsCount = engine.tops.filter(t => !t.isAI && !t.isExploding && t.hp > 0 && (Math.hypot(t.x - pad.x, t.y - pad.y) < (t.radius + padRadius) * 0.75)).length;

                if (waitingTopsCount > 0) {
                    ctx.save();
                    // Draw waitingTopsCount / X pie slices with the rotating rainbow gradient
                    for (let i = 0; i < waitingTopsCount; i++) {
                        const startAngle = -Math.PI / 2 + (i * Math.PI * 2 / X);
                        const endAngle = -Math.PI / 2 + ((i + 1) * Math.PI * 2 / X);

                        ctx.beginPath();
                        ctx.moveTo(pad.x, pad.y);
                        ctx.arc(pad.x, pad.y, padRadius, startAngle, endAngle);
                        ctx.closePath();
                        
                        ctx.fillStyle = rainbowGrad;
                        ctx.globalAlpha = 0.85; // highly visible rainbow
                        ctx.fill();

                        // sleek border around each pie slice
            ...
            ctx.lineWidth = isMulti ? 2.5 : 1.5;
            ctx.beginPath();
            ctx.arc(pad.x, pad.y, padRadius + 14, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();
            ...
            ctx.lineWidth = isMulti ? 7.5 : 4.5; // Thicker stroke for bigger pad
            ctx.beginPath();
            ctx.arc(pad.x, pad.y, padRadius + 6, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();
            ...
            ctx.save();
            ctx.strokeStyle = rainbowGrad2;
            ctx.lineWidth = isMulti ? 3.5 : 2.0; // Sharp inner neon wire
            ctx.beginPath();
            ctx.arc(pad.x, pad.y, padRadius - 3, 0, Math.PI * 2);
            ctx.stroke();
            ctx.restore();

            if (isMulti) {
                // ================= 1.5 FOUR INWARD POINTING ROTATING ARROWS =================
                const arrowGroupRotation = -(Date.now() / 1200) % (Math.PI * 2);
                const arrowOrbitRadius = padRadius + 40; // Push further out for larger arrows
                ctx.save();
                ctx.translate(pad.x, pad.y);
                ctx.rotate(arrowGroupRotation);
                ...
                for (let aIdx = 0; aIdx < 4; aIdx++) {
                    ctx.save();
                    ctx.rotate((Math.PI / 2) * aIdx);
                    ctx.translate(0, -arrowOrbitRadius);
                    ctx.scale(2, 1); // Double width, normal height (squashed by 50% compared to previous 2x)
                    
                    // Draw 3 layers of chevrons pointing down (towards center)
                    for (let layer = 0; layer < 3; layer++) {
                        const yOffset = layer * -10; // spacing between chevrons
                        ctx.beginPath();
                        ctx.moveTo(-14, -16 + yOffset);
                        ctx.lineTo(0, -2 + yOffset);
                        ctx.lineTo(14, -16 + yOffset);
            ...
            // ================= 2. SELF-ROTATING HIGH-BRIGHTNESS RAINBOW STAR SYMBOLS =================
            const activePlayerCountForStars = engine.tops.filter(t => !t.isAI && !t.isExploding && t.hp > 0).length;
            const starCount = isMulti ? Math.max(1, activePlayerCountForStars) : 1;
            const orbitRadius = isMulti ? 65 : 0;
            const orbitRotation = isMulti ? (Date.now() / 1500) % (Math.PI * 2) : 0;
            ...
            for (let sIdx = 0; sIdx < starCount; sIdx++) {
                ...
                if (isMulti) {
                    const angle = orbitRotation + (sIdx * Math.PI * 2 / starCount);
                    starX += Math.cos(angle) * orbitRadius;
                    starY += Math.sin(angle) * orbitRadius;
                }
                
                ctx.translate(starX, starY);
                
                // Auto rotation for star
                const starRotation = (Date.now() / (isMulti ? 400 : 600)) % (Math.PI * 2);
                ctx.rotate(starRotation);
                
                // Dynamic rainbow gradient for the star fill
                const starAngleRange = (Date.now() / 350) % (Math.PI * 2);
                const gradientSize = isMulti ? 26 : 26;
                const rCos = Math.cos(starAngleRange) * gradientSize;
                const rSin = Math.sin(starAngleRange) * gradientSize;
                ...
                const spikes = 5;
                const outerR = isMulti ? 26 : 24;
                const innerR = isMulti ? 11 : 10;
                ...
                const smallOuterR = isMulti ? 11 : 11;
                const smallInnerR = isMulti ? 4.5 : 4.5;
                ctx.beginPath();

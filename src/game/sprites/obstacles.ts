...
export function createChestSprite(): HTMLCanvasElement {
    const c = document.createElement('canvas');
    c.width = 30 * SCALE; 
    c.height = 30 * SCALE; 
    const ctx = c.getContext('2d')!;
    ...
    // Outer glow for the treasure chest
    ctx.shadowColor = 'rgba(250, 204, 21, 0.6)';
    ctx.shadowBlur = 15;
    
    // Draw a hexagonal treasure box shape
    ctx.beginPath();
    const sides = 6;
    const rBase = 12 * SCALE;
    for (let i = 0; i < sides; i++) {
        const ang = (Math.PI * 2 / sides) * i;
        const x = cx + Math.cos(ang) * rBase;
        const y = cy + Math.sin(ang) * rBase;
    ...
    const grad = ctx.createLinearGradient(cx - rBase, cy - rBase, cx + rBase, cy + rBase);
    grad.addColorStop(0, '#fef08a'); // light gold
    grad.addColorStop(0.5, '#eab308'); // gold
    grad.addColorStop(1, '#a16207'); // dark gold
    ...
    // Inner bevel ring
    ctx.beginPath();
    ctx.arc(cx, cy, rBase * 0.7, 0, Math.PI * 2);
    ...
    // Center lock/gem
    ctx.beginPath();
    ctx.arc(cx, cy, rBase * 0.3, 0, Math.PI * 2);

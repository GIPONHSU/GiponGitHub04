...
export function hitZombie(
  ...
  const isClientControlling = !top.isAI && top.controls && top.struggleMashCount === undefined;
  ...
  */
  ...
  if (z.hp <= 0) {
    ...
    if (!isAlreadyDying) {
      ...
      // 2. Drop Ticket happens later in zombieBehavior.ts when dying animation finishes
...
export function handleWallBounce(engine: GameEngine, e: Entity): boolean {

...
export function spawnChest(engine: GameEngine) {
  // Only 1 chest at a time
  if (engine.obstacles.some((o) => o.type === 'obstacle_chest' && !o.markForDeletion)) {
    return;
  }
  ...
  for (let attempts = 0; attempts < 150; attempts++) {
    ...
    if (GameUtils.isPointInsideCapsule(engine, bx, by, 50)) {
      ...
      const minObstacleDistance = 140 * bufferMultiplier;
      ...
      for (const block of engine.concreteBlocks) {
        ...
        const minDistX = (block.w / 2 + 100) * bufferMultiplier;
        const minDistY = (block.h / 2 + 100) * bufferMultiplier;

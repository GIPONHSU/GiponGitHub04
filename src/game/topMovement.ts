...
import { checkCircleBoxCollision, resolveCircleBoxCollision } from './physics';
    ...
    export function updateTopStandby(top: Top, engine: GameEngine, dt: number) {
        ...
        const isStruggling = top.struggleMashCount !== undefined;
        
        if (!top.isAI && top.controls) {
            const isClinging = (engine.zombieSiegeActive && (engine.siegeStatus === 'clinging' || engine.siegeStatus === 'resolved_success') && top.id === engine.siegeTargetPlayerId) || top.launchPadState !== undefined || (top.breakoutOrbitTimer !== undefined && top.breakoutOrbitTimer > 0) || engine.transitionState === 'buffer' || isStruggling;

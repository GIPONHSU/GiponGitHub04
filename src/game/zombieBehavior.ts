...
export function updateZombies(
  ...
  // Gather targets currently locked by any black zombie
  const blackZombieTargets = new Set<string>();
  this.zombies.forEach((z) => {
    if (z.type === "zombie_black" && !z.markForDeletion) {
      const bz = z as any;
      if (bz.bossWarningTargetId && bz.bossAttackState !== 'idle') {
        blackZombieTargets.add(bz.bossWarningTargetId);
      }
    }
  });

  const otherZombieTargets = zombieTargets.filter(t => !blackZombieTargets.has(t.id));
  ...
  this.zombies.forEach((z) => {
    ...
    if (
      ...
      if ((z as any).dyingTimer <= 0) {
        ...
        // Spawn rainbow concentric shockwaves specifically for big, bomb, and bouncing zombies
        if (z.type === "zombie_big" || z.type === "zombie_bomb" || z.type === "zombie_bouncing") {
    ...
    if (z.hp <= 0 && !z.markForDeletion && !(z as any).isDying) {
      ...
      const isBig =
        (z.type as string) === "zombie_big" ||
        (z.type as string) === "zombie_bouncing" ||
      ...
      if (isBoss) {
        ...
        mainColor = "#374151";
        secondColor = "#111827";
      } else if ((z.type as string) === "zombie_big") {
        mainColor = "#9333ea";
        secondColor = "#6b21a8";
      } else if ((z.type as string) === "zombie_bouncing") {
        mainColor = "#be185d";
        secondColor = "#831843";
      }
    ...
    if (z.type === "zombie_boss") {
      ...
      updateBossZombie(this, z, dt, otherZombieTargets);
      return;
    }
    ...
    if (z.type === "zombie_big") {
      ...
      updateBigZombie(this, z, dt, otherZombieTargets);
      return;
    }

    if (z.type === "zombie_bomb") {
      // Handled in BombZombie.ts
      updateBombZombie(this, z, dt, otherZombieTargets);
      return;
    }

    if (z.type === "zombie_bouncing") {
      // Handled in BouncingZombie.ts
      updateBouncingZombie(this, z, dt, otherZombieTargets);
      return;
    }
    ...
    if (z.type === "zombie_golden") {
      updateGoldenZombie(this, z, dt, otherZombieTargets);
      return;
    }

    // Handled in BasicZombie.ts
    updateBasicZombie(this, z, dt, otherZombieTargets);
